...
const classroomOnly = process.argv.slice(3).length > 0 && process.argv.slice(3).every(name => ['setup','lesson'].includes(name));
const key = (classroomOnly ? process.env.FDE_CAMP_CLASSROOM_KEY : process.env.FDE_CAMP_20260913_KEY) || '';
...
for (const name of (names.length ? names : ['book', 'slides'])) {
  if (!['book','slides','setup','lesson'].includes(name)) throw new Error('Unknown course page');
