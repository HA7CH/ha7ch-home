...
Edit approved copy and cards in `src/app/anc/board.tsx`, presenter details in `presenter.ts`, and layout in `board.module.css`. Default canvas is 3:4; Shift+V toggles the presentation aspect ratio. F toggles browser fullscreen; Escape exits fullscreen. Keyboard shortcuts ignore text inputs and modifier combinations. Cards expand in place. Camera access is requested only after clicking the camera control; video stays local and is not uploaded.
...
Verify both overview and expanded cards at desktop and narrow viewport sizes after changes. Preserve approved copy when adjusting layout.
