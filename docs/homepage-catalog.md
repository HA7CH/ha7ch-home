...
2026-09-07 使用 mee7 CLI 全量核对现有官网活动的状态：48H FDE Sprint、HCN Creator 首期计划及北京 FDE PRO 快照从 open 同步为 closed；上海 FDE Night 与 ANC Fund 保持 open。本次仅同步状态，保留全量快照 syncedAt 和项目数据，避免将未刷新的字段标为已同步。无日期的关闭入口显示“报名已关闭”，不再展示滚动招募或申请回复时效。
