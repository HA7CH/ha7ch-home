...
>
> 状态：v1。命名已定：ha7ch news，域名 news.ha7ch.com（2026-07-10 lawted 拍板，致敬 news.ycombinator.com；曾用工作名 dispa7ch，本文正文中的 dispa7ch 均指本产品）。综合四份研究（HN 架构落地 / 竞品与 AI-native 差异化 / 放置决策 / 命名评审）。
>
...
### 名字：已定 ha7ch news（下表为当时的评审记录，存档）

| 排名 | 名字 | 子域 | 一句话理由 |
|---|---|---|---|
| **主选** | **dispa7ch** | news.ha7ch.com | dispatch = 从 field 发回的一手战报，与 slogan「Build in the field」同一根血管。既是论坛动作（发一条 dispatch）又是身份叙事，对 HN（泛科技）和 mee7（活动）都硬区隔。代价：8 字母偏长、7 藏词中，不伤使用。 |
...
最终定名 **ha7ch news**：lawted 判断 dispa7ch 不易记，通用词骑在 ha7ch 品牌上记忆成本最低，且 news.ha7ch.com 与 news.ycombinator.com 形成结构性致敬。正文沿用 dispa7ch 指代产品（历史文本），运营引用以本节为准。
...
**独立仓库（HA7CH/ha7ch-news）→ 独立 Vercel 项目 → news.ha7ch.com，用独立的 Supabase project（不复用 mee7 那个）。** 作为第四个独立产品加入 `X.ha7ch.com` 家族。
...
```
HA7CH/ha7ch-news  (private repo → Vercel project → news.ha7ch.com)
...
主站接入：`ha7ch-home/src/app/page.tsx` 的 projects 数组加一条，href 指 `https://news.ha7ch.com`，零运行时耦合。这正是 mee7 现在的接入方式。
...
- **每个 thread 提供 `.md` 视图**（`news.ha7ch.com/t/{id}.md`），干净可喂。
...
| 部署 | 独立仓 → news.ha7ch.com + 独立 Supabase project | mee7 先例（更进一步:独立 DB） | 塞进 ha7ch-home 主仓 |
