...
const article: Article = {
  ...
  zh: [
    ...
    "---",
    "Genie Space",
    "Genie Space 是一个自然语言交互，使用 SQL 从数据库获取对应数据，并简单可视化展示的 Data Agent。这个概念本身看起来很好，很吸引人，但是 AI 获取数据的准确性和稳定性堪忧。尤其是在复杂要求和多数据库联合时有待加强。",
    "Genie Space 提供客制化修改指令和范式的功能。这是在一个底层的技术黑盒上搭建范式，不透明的底层指令和构建的客制化指令相冲突，让本应该加强能力的设置反而成为了阻碍。",
    "例如：当询问「过去一个月的销售额」，语义可以有很多种解释，是上个月，还是过去 30 天，亦或者是月初到今天。尽管在客制化的指令中定义了「过去一个月」是指上个月，但是如果和底层黑盒中的设置指令相冲突，模型会随机根据一种定义选择回答。",
    ...
    "严肃一点来说，Genie Code 在复杂工程环境中艰难达到及格线。在 Databricks 的 AI Day 演示上，Databricks 官方员工使用 Claude Code 搭建 MVP 运行在自家平台上，而不是使用 Genie Code。",
    ...
    "以我个人的愚见，大力加强 Genie Code，使其达到和 GitHub Copilot 同样的水准，结合 Databricks 的数据库，作为超级个体提供放大能力的 AI 工具，缩小和现阶段 T0 级别 AI 工具的差距。保持 Genie Code 的云端优势，同时给用户一定的自由度来修改工具本身。",
  ...
  en: [
    ...
    "To put it more seriously, Genie Code barely reaches the passing line in complex engineering environments. In Databricks' AI Day demo, Databricks' own staff used Claude Code to build an MVP running on their platform, instead of using Genie Code.",
