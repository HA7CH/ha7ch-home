...
const article: Article = {
  ...
  zh: [
    ...
    "VibeCoding 正在成为很多工程师日常开发的基础设施。当你在某次长任务中撞上了 rate limit 或者 context 上限的时候，最直接的解决办法当然是给 GPT 冲一个 200 刀的 Pro、给 Claude 充一个 200 刀 Max——问题立刻消失。",
    "但如果我们不从财力上绕开这个问题，而是从原理上真正问一句「token 到底去哪了」，你难道不好奇吗？反正我是很好奇。",
    "---",
    "先说计费机制。目前主流的 LLM API，包括 Claude 和 OpenAI，都按 token 数量计费，区分输入和输出。输入会便宜一点，输出会贵一些。在Code Agent这个具体的情景之下，Input的token消耗占绝对大头，甚至达到 80% 以上。",
    ...
    "代价落在token消耗上，最终落在消费者身上。Token消耗的重灾区是上下文管理，这里只能用「A Piece of SHIT」来形容。",
    ...
    "---",
    "本人对这个 Field 持开放态度，目前来说站Model Level。一个是马后炮唯结果论的原因，比较Harnesss Level的方法现在工业界一个都没有用上，那总有他的原因在。还有一个是推广度上的看法，如果从Model Level解决了问题那么就会像这次DeepSeek一样，无需过多推广，缺token的人自然会使用你的API。",
    "更何况从厂商的角度来说，token的减耗是暂时的，时间和算力的减耗是永远的。",
    "任何观点和方法都可以联系我们进行探讨。",
  ],
  en: [
